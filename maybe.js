...
function isJust<A>(fa: Maybe<A>): boolean {
  return fa(_ => true, () => false);
}

function isNothing<A>(fa: Maybe<A>): boolean {
  return fa(_ => false, () => true);
}

function map<A, B>(f: A => B): (Maybe<A>) => Maybe<B> {
  // return (fa: Maybe<A>) => fa((a: A) => just(f(a)), nothing);
...
const _nothing: Maybe<number> = nothing();
...
function divBy(a: number) {
  return function(b: number): Maybe<number> {
    if (a === 0) {
      return nothing();
    } else {
      return just(b / a);
